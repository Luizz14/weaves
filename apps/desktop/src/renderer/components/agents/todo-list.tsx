...
function statusLabel(status: TodoItemStatus) {
	if (status === "in-progress") return i18n._(msg({ message: "In progress" }));
	if (status === "completed") return i18n._(msg({ message: "Completed" }));
	if (status === "cancelled") return i18n._(msg({ message: "Cancelled" }));
	return i18n._(msg({ message: "Pending" }));
}
...
export function TodoList({
	...
	return (
		<section
			aria-label={i18n._(msg({ message: "Plan" }))}
			className={cn(
			...
			>
				...
				>
					<span className="sr-only">
						{i18n._(msg({ message: `${completed} / ${items.length}` }))}
					</span>
				...
				>
					{items.length ? (
						...
						<p className="px-1.5 py-2 text-sm text-muted-foreground">
							{i18n._(msg({ message: "No tasks yet" }))}
						</p>
					)}
				</div>
			</AgentDisclosure>
