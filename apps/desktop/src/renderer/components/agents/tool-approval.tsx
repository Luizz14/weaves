...
import { EASE_OUT, SPRING_PRESS, SPRING_SWAP } from "renderer/lib/ease";
...
function getStatusCopy(status: ToolApprovalStatus) {
	if (status === "approving") return i18n._(msg({ message: "Approving" }));
	if (status === "approved") return i18n._(msg({ message: "Approved" }));
	if (status === "denied") return i18n._(msg({ message: "Denied" }));
	if (status === "running") return i18n._(msg({ message: "Running" }));
	if (status === "complete") return i18n._(msg({ message: "Completed" }));
	if (status === "error") return i18n._(msg({ message: "Failed" }));
	return i18n._(msg({ message: "Approval required" }));
}
				...
				{pending ? (
					...
					>
						...
						{onAlwaysAllow ? (
							...
							>
								{i18n._(msg({ message: "Allow for session" }))}
							</motion.button>
						) : null}
						<button
							type="button"
