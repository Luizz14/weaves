...
function MobileSidebar({
	...
	return createPortal(
		...
		>
			<motion.button
				type="button"
				aria-label={i18n._(msg({ message: "Close sidebar" }))}
	...
	function AnimatedSidebar(
		{
			...
			variant = "sidebar",
			collapsible = "icon",
			ariaLabel = i18n._(msg({ message: "Sidebar" })),
			children,
			className,
			panelClassName,
			style,
...
>(function AnimatedSidebarTrigger(
	...
	return (
		<button
			...
			type={type}
			aria-label={
				props["aria-label"] ?? i18n._(msg({ message: "Toggle sidebar" }))
			}
...
>(function AnimatedSidebarClose(
	...
	return (
		<button
			...
			type={type}
			aria-label={
				props["aria-label"] ?? i18n._(msg({ message: "Close sidebar" }))
			}
			onClick={(event) => {
				onClick?.(event);
...
>(function AnimatedSidebarRail(
	...
	return (
		<button
			...
			title={i18n._(msg({ message: "Toggle sidebar" }))}
			tabIndex={-1}
			onClick={(event) => {
				onClick?.(event);
				if (!event.defaultPrevented) context.toggleSidebar();
