...
test("queued prompts retain their execution settings across a later configuration change", async () => {
	const { FakeHarness } = await import("../../harness/fake");
	const received: Array<
		import("@superset/chat/protocol").CodexExecution | undefined
	> = [];
	class ConfigurableHarness extends FakeHarness {
		async configureCodex(
			execution: import("@superset/chat/protocol").CodexExecution,
		) {
			return execution;
		}
		override prompt(
			content: import("@superset/chat/protocol").UserContent[],
			execution?: import("@superset/chat/protocol").CodexExecution,
		) {
	...
	const adapter = new ConfigurableHarness({
		turns: [
			[
				{ kind: "turn", turn: turn("one") },
				{
					kind: "turn",
					turn: turn("one", { status: "completed" }),
					delayMs: 20,
				},
			],
			[
				{ kind: "turn", turn: turn("two") },
				{ kind: "turn", turn: turn("two", { status: "completed" }) },
			],
		],
	});
	const runtime = createTestRuntime({
		harnesses: new Map([["configured", () => adapter]]),
	});
	const { sessionId } = runtime.commands.createSession({
		commandId: randomUUID(),
		scopeId: "workspace",
	...
	try {
		await runtime.commands.configureCodex({
			commandId: randomUUID(),
			sessionId,
			execution,
		});
		...
		await runtime.commands.configureCodex({
			commandId: randomUUID(),
			sessionId,
			execution: { ...execution, modelId: "astra" },
		});
		await waitFor(() => received.length === 2);
		expect(received.map((value) => value?.modelId)).toEqual(["luna", "luna"]);
	} finally {
		await runtime.dispose();
	}
});
